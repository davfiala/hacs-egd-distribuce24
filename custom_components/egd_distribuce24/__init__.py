...
from .hdo_coordinator import HdoCoordinator
...
async def async_setup_entry(hass, entry):
    ...
    coordinator.hdo = None
    if entry.data.get("hdo_settings"):
        coordinator.hdo = HdoCoordinator(hass, entry, async_get_clientsession(hass))
        # A temporary HDO outage must not block measured energy or its entities.
        await coordinator.hdo.async_refresh()
